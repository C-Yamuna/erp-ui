...
import { SiBorrowingService } from './shared/si-borrowing.service';
import { Router } from '@angular/router';
import { CommonComponent } from 'src/app/shared/common.component';
import { BorrowingTransactionConstant } from '../borrowing-transaction-constants';
import { applicationConstants } from 'src/app/shared/applicationConstants';
import { DatePipe } from '@angular/common';
import { BorrowingConfigConstants } from 'src/app/configurations/borrowing-config/borrowing-config-constants';
import { CommonStatusData } from '../../common-status-data.json';
import { SiBorrowing } from './shared/si-borrowing.model';

@Component({
  selector: 'app-si-borrowing',
  templateUrl: './si-borrowing.component.html',
  styleUrls: ['./si-borrowing.component.css']
})
export class SiBorrowingComponent {
  borrowings: any[] = [];
  statuses!: SelectItem[];
  operations:any;
  ...
  constructor(private router: Router,private translate: TranslateService,private commonFunctionsService: CommonFunctionsService
    ,private siborrowingsService : SiBorrowingService , private encryptDecryptService: EncryptDecryptService,private datePipe: DatePipe,
...
newBorrrowing(){
  this.router.navigate([BorrowingTransactionConstant.SI_ACCOUNT_DETAILS]);
}
editborrowing(rowData: any){
  this.router.navigate([BorrowingTransactionConstant.SI_VIEW_BORROWING], { queryParams: { id: this.encryptDecryptService.encrypt(rowData.id),editbtn: this.encryptDecryptService.encrypt(applicationConstants.ACTIVE), isGridPage: this.encryptDecryptService.encrypt(applicationConstants.IN_ACTIVE) } });
}

view(rowData: any) {
  let viewScreen = true;
  this.router.navigate([BorrowingTransactionConstant.SI_VIEW_BORROWING], { queryParams: { id: this.encryptDecryptService.encrypt(rowData.id),view: this.encryptDecryptService.encrypt(viewScreen), editbtn: this.encryptDecryptService.encrypt(applicationConstants.IN_ACTIVE),isGridPage: this.encryptDecryptService.encrypt(applicationConstants.IN_ACTIVE) }});
 }
navigateToInfoDetails(rowData:any,event: any) {
  if (event.value === 1)
  this.router.navigate([BorrowingTransactionConstant.SI_DISBURSEMENT],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
  else if (event.value === 2)
  this.router.navigate([BorrowingTransactionConstant.SI_COLLECTION],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
  else if (event.value === 3)
  this.router.navigate([BorrowingTransactionConstant.SI_CLOSURE],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
  else if (event.value === 4)
  this.router.navigate([BorrowingTransactionConstant.SI_CHARGES_COLLECTION],{ queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
 

}
/**
   * @author vinitha
   * @implements get all by pacsId and branchId(grid data)
   */
getAllBorrowingsAccountDetailsListByPacsIdAndBranchId() {
  this.commonComponent.startSpinner();
 this.orgnizationSetting = this.commonComponent.orgnizationSettings();
 this.siborrowingsService.getSiBorrowingAccountsListByPacsIdAndBranchId(this.pacsId,this.branchId).subscribe((data: any) => {
