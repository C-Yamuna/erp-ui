...
export class TermLoanProductDefinitionService {
 ...
 getPreviewDetailsByProductId(id: string) {
  ...
  downloadTermPreviewPDf(id:any){
    let status = this.commonFunctionService.getStorageValue('language');
    let url = ERP_TRANSACTION_CONSTANTS.LOANS+ERP_TRANSACTION_CONSTANTS.TERM_PRODUCT_DEFINITIONS+ERP_TRANSACTION_CONSTANTS.TERM_LOAN_PRODUCT_DEFINITION_PREVIEW_DOWNLOAD+"/"+id + "/"+status;
