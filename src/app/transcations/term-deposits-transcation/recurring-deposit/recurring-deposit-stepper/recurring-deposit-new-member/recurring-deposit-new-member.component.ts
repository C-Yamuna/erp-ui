...
export class RecurringDepositNewMemberComponent {
  ...
  ngOnInit(): void {
   ...
   getAllSubProducts() {
     this.commonComponent.startSpinner();
     this.rdAccountsService.getAllSubProduct().subscribe((res: any) => {
           ...
           if (this.responseModel.data[0] != null && this.responseModel.data[0] != undefined) {
             if (this.responseModel.status === applicationConstants.STATUS_SUCCESS) {
               if (this.responseModel.data.length > 0 && this.responseModel.data[0] != null && this.responseModel.data[0] != undefined) {
                 this.admisionNumber = this.responseModel.data[0].adminssionNumber;
   ...
   getCastesList() {
    ...
    /**
     ...
     OnChangeMemberType(event: any) {
       ...
       if (event.value == 1) {
         ...
         this.rdAccountsModel.memberShipGroupDetailsDTO = null;
         this.membershipGroupDetailsModel = new MemberGroupDetailsModel();
         this.membershipInstitutionDetailsModel = new MembershipInstitutionDetailsModel();
         this.rdAccountsModel.memInstitutionDTO = null;
         ...
         this.rdAccountsModel.memberShipBasicDetailsDTO = null;
         this.rdAccountsModel.memInstitutionDTO = null;
         this.memberCreationForm.reset();
         this.institutionForm.reset();
     ...
     */
     savePromoterDetails(rowData: any) {
       ...
       if (!this.membershipGroupDetailsModel.groupPromoterList) {
         this.membershipGroupDetailsModel.groupPromoterList = []; // Initialize it as an empty array
       }
   ...
   fileUploader(event: any, fileUpload: FileUpload, filePathName: any) {
     ...
     if (filePathName === "individualResolutionCopy") {
       this.membershipBasicDetailsModel.multipartFileListForResolutionCopyPath = [];
       if (selectedFiles[0].size/1024/1024 > 2) { 
       ...
       for (let file of selectedFiles) {
         ...
         reader.onloadend = (e) => {
           ...
           if (filePathName === "individualPhotoCopy") {
             ...
             this.membershipBasicDetailsModel.filesDTOList.push(files);
             this.membershipBasicDetailsModel.multipartFileListForResolutionCopyPath.push(files);
             this.membershipBasicDetailsModel.resolutionCopy = null;
             this.membershipBasicDetailsModel.filesDTOList[this.membershipBasicDetailsModel.filesDTOList.length - 1].fileName = "Resulution_copy" + "_" + timeStamp + "_" + file.name;
             this.membershipBasicDetailsModel.resolutionCopy = "Resulution_copy" + "_" + timeStamp + "_" + file.name; // This will set the last file's name as docPath
           }
     ...
     fileRemoeEvent(fileName: any) {
         if (this.membershipBasicDetailsModel.filesDTOList != null && this.membershipBasicDetailsModel.filesDTOList != undefined && this.membershipBasicDetailsModel.filesDTOList.length > 0) {
           ...
           let removeFileIndex = this.membershipBasicDetailsModel.filesDTOList.findIndex((obj: any) => obj && obj.fileName === this.membershipBasicDetailsModel.photoPath);
           let obj = this.membershipBasicDetailsModel.filesDTOList.find((obj: any) => obj && obj.fileName === this.membershipBasicDetailsModel.photoPath);
         ...
         if (fileName == "individualSighnedCopy") {
           ...
           let removeFileIndex = this.membershipBasicDetailsModel.filesDTOList.findIndex((obj: any) => obj && obj.fileName === this.membershipBasicDetailsModel.signaturePath);
           let obj = this.membershipBasicDetailsModel.filesDTOList.find((obj: any) => obj && obj.fileName === this.membershipBasicDetailsModel.signaturePath);
           this.membershipBasicDetailsModel.filesDTOList.splice(removeFileIndex, 1);
           this.membershipBasicDetailsModel.signaturePath = null;
         }
         this.updateData();
   ...
   ageCaluculation(flag: any) {
     if (flag) {//with age to date convertion
       ...
       if (this.membershipBasicDetailsModel.dobVal) {
         const dob = new Date(this.membershipBasicDetailsModel.dobVal);
         ...
         this.membershipBasicDetailsModel.age = age;
       }
     }
   ...
   groupTypeOnChange(){
     ...
     if (group != null && undefined != group)
       this.membershipGroupDetailsModel.groupName = group.label;
   }
