...
import { InterestPayment } from '../../interest-payment/shared/interest-payment.model';
import { InterestPaymentService } from '../../interest-payment/shared/interest-payment.service';
import { FormBuilder, FormControl, FormGroup, Validators } from '@angular/forms';
import { FdNonCummulativeInterestPayment, FdNonCummulativeTransaction } from './shared/fd-non-cummulative-interest-payment.model';
import { FdNonCummulativeInterestPaymentService } from './shared/fd-non-cummulative-interest-payment.service';
import { Table } from 'primeng/table';

@Component({
  selector: 'app-fd-non-cumulative-interest-payment',
  templateUrl: './fd-non-cumulative-interest-payment.component.html',
  styleUrls: ['./fd-non-cumulative-interest-payment.component.css']
})
export class FdNonCumulativeInterestPaymentComponent {
  ...
  ngOnInit() {
    ...
    // this.fdNonCummulativeTransactionModel.interestPostingDate = this.currentDate();
    // this.getAllInterestPayment();
    // this.interestPaymentModel.transactionMode = this.interestPaymentForm.get('transactionMode')?.value.value;
  }
...
*/
  ...
  imageUploader(event: any, fileUpload: FileUpload) {
    ...
    for (let file of event.files) {
      ...
      reader.onloadend = (e) => {
        ...
        if (index === -1) {
          this.multipleFilesList.push(files);
          this.fdNonCummulativeTransactionModel.filesDTOList.push(files); // Add to filesDTOList array
        }
        let timeStamp = this.commonComponent.getTimeStamp();
        this.fdNonCummulativeTransactionModel.filesDTOList[0].fileName = "FD_NON_KYC_" + this.termAccId + "_" +timeStamp+ "_"+ file.name ;
        this.fdNonCummulativeTransactionModel.signedCopyPath = "FD_NON_KYC_" + this.termAccId + "_" +timeStamp+"_"+ file.name; // This will set the last file's name as docPath
  ...
  getAllPaymentsByAccountIdAndAccountNumber() {
    this.commonComponent.startSpinner();
    this.interestPaymentService.getAllPaymentsByAccountIdAndAccountNumber(this.termAccId,this.accountNumber).subscribe((data: any) => {
      this.responseModel = data;
      if (this.responseModel.status === applicationConstants.STATUS_SUCCESS) {
        this.interestPaymentList = this.responseModel.data.map((item: { interestPostingDate: string | number | Date | null; }) => {
