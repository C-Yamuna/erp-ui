...
import { AssociatedBankDetailsService } from '../associated-bank-details/shared/associated-bank-details.service';
...
export class InterestPolicyComponent {
  ...
  { 
    this.interestPolicyForm = this.formBuilder.group({
      ...
      'penalRoi': new FormControl('', [Validators.pattern(applicationConstants.ALLOW_TWO_NUMBERS_TWO_DECIMALS),Validators.required]),
  ...
  addNewEntry()
  {
    this.newRow = {generalRoi: '', staffRoi: '', seniorcitizenRoi: '',penaltyRoi: '', effectiveStartDate: ''}
  }
     /**
    @author Bhargavi
  ...
  getInterestpolicyDetails(id: any) {
    ...
    this.investmentsProductDefinitionService.getPreviewByProductId(id).subscribe(res => {
      ...
      if (this.responseModel.status == applicationConstants.STATUS_SUCCESS && this.responseModel.data[0] != null) {
        ...
        if (this.investmentsProductDefinitionModel.interestPolicyConfigDTO != null && this.investmentsProductDefinitionModel.interestPolicyConfigDTO != undefined &&
            this.investmentsProductDefinitionModel.interestPolicyConfigDTO.length > 0) {

              this.enableSaveAndNextButton = applicationConstants.TRUE;

          this.intrestpolicyList = this.investmentsProductDefinitionModel.interestPolicyConfigDTO;
  ...
  //   rowData.effectiveStartDate = this.commonFunctionService.getUTCEpoch(new Date(rowData.effectiveStartDate));
   ...
   if(rowData.id != null)
    {
        this.interestPolicyService.updateInterestPolicyConfig(rowData).subscribe((response: any) => {
    ...
    rowData.status = applicationConstants.ACTIVE;
     this.interestPolicyService.addInterestPolicyConfig(rowData).subscribe((response: any) => {
       this.responseModel = response;
       if (this.responseModel.status === applicationConstants.STATUS_SUCCESS && this.responseModel.data[0] !=null) {
        if (this.responseModel != null&& this.responseModel.data!= undefined) {
