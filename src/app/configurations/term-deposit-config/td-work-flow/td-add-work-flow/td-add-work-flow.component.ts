...
import { WorkFlowStepModel, WorkFlowSteps } from '../shared/work-flow-step-model';
...
@Component({
  selector: 'app-td-add-work-flow',
  templateUrl: './td-add-work-flow.component.html',
  styleUrls: ['./td-add-work-flow.component.css']
})
export class TdAddWorkFlowComponent implements OnInit{
  ...
  constructor(private formBuilder: FormBuilder, private datePipe: DatePipe, private router: Router,
    ...
    private commonComponent: CommonComponent, private workFlowStepService: WorkflowService) { 
      this.projectTaskForm = this.formBuilder.group({
        'commonStatusId': new FormControl('', [Validators.required]),
    ...
    ngOnInit() {
      this.workFlowColumns = [
        { field: 'commonStatusName', header: 'TERMDEPOSITS.NAME' },
        { field: 'activityName', header: 'TERMDEPOSITS.DESCRIPTION' },
        { field: 'prevStepName', header: 'TERMDEPOSITS.STATUS' },
    ...
    //add inline row
    addProjectRow() {
      return { commonStatusId: '', level: '', prevStepId: '', isActive: applicationConstants.TRUE }
    }
    ...
    backToWorkFlowStep() {
      this.router.navigate([TERMDEPOSITCONFIGCONSTANTS.WORK_FLOW])
    }
    ...
    checkDuplicatePreviousSteps(obj: any) {
        ...
        if (index != -1) {
          ...
          this.projectTaskForm.get("commonStatusId").reset();
          this.msgs = [];
