...
import { AGENTDETAILSCONFIGCONSTANTS } from '../../agent-details-config/agent-details-config-constants';
...
export class DocumentTypesComponent {
  ...
  addkycdoctype() {
    this.router.navigate([TERMDEPOSITCONFIGCONSTANTS.ADD_DOCUMENT_TYPE]);
  }
  getAll() {
  ...
  editkycdoctype(rowData: any) {
    this.router.navigate([TERMDEPOSITCONFIGCONSTANTS.ADD_DOCUMENT_TYPE], { queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
  }
