...
export class AddWorkFlowComponent {
  ...
  ngOnInit() {
    this.workFlowColumns = [
      { field: 'commonStatusName', header: 'DAILYDEPOSITSCONFIG.NAME' },
      { field: 'activityName', header: 'DAILYDEPOSITSCONFIG.DESCRIPTION' },
      { field: 'prevStepName', header: 'DAILYDEPOSITSCONFIG.STATUS' },
  ...
  backToWorkFlowStep() {
    this.router.navigate([DailyDepositConfigConstants.WORK_FLOW])
  }
  ...
  checkDuplicatePreviousSteps(obj: any) {
      ...
      if (index != -1) {
        ...
        this.msgs = [];
        this.msgs = [{ severity: 'error', summary: applicationConstants.STATUS_ERROR, detail: "CommonStatus Already Existed" }];
        setTimeout(() => {
          this.msgs = [];
        }, 2000);
      }
