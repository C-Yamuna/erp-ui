...
export class ServiceTypesComponent {
  ...
  constructor(private router: Router, private serviceTypesService: ServiceTypesService,
    ...
    this.columns = [
      ...
      { field: 'cgstPercentage', header: 'DEMANDDEPOSITS.CGST' },
      { field: 'sgstPercentage', header: 'DEMANDDEPOSITS.SGST' },
  ...
  addservicetype() {
    this.router.navigate([SavingsBankConfigConstants.ADD_SERVICE_TYPE]);
  }
  getAll() {
  ...
  editservicetypes(rowData: any) {
    this.router.navigate([SavingsBankConfigConstants.ADD_SERVICE_TYPE], { queryParams: { id: this.encryptDecryptService.encrypt(rowData.id) } });
  }
